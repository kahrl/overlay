...
MY_P="${PN}-${MY_PV}"
MY_CHANGESET="b277d01700f6"

RESTRICT="mirror"

DESCRIPTION="A minecraft clone"
HOMEPAGE="http://celeron.55.lt/~celeron55/minetest/"
SRC_URI="https://bitbucket.org/celeron55/${MY_PN}/get/${MY_PV}.tar.gz -> ${MY_P}.tar.gz"
...
src_prepare() {
	epatch "${FILESDIR}"/minetest-find-share-games.patch
}
...
src_install() {
	...
	if use client; then
		...
		dogamesbin "${CMAKE_BUILD_DIR}"/bin/minetest
		...
		if use nls; then
			# There is a po/en directory, but it only contains an incomplete
			# .pot file, so we never install it.
			einfo "Installing .mo files"
			for MY_LINGUA in ${MY_LINGUAS}; do
				if use linguas_${MY_LINGUA/-/_}; then
					insinto /usr/share/locale/${MY_LINGUA}/LC_MESSAGES
					doins "${CMAKE_BUILD_DIR}"/locale/${MY_LINGUA}/LC_MESSAGES/${MY_PN}.mo
				fi
			done
		fi
	fi
	...
	if use server; then
		...
		dogamesbin "${CMAKE_BUILD_DIR}"/bin/minetestserver
	fi
